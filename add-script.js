...
class AddSongManager {
  ...
  async autoFetchLyrics() {
    ...
    try {
      const lyrics = await this.fetchLyricsFromAPI(artist, title);

      if (lyrics) {
        document.getElementById("songLyrics").value = lyrics;
      ...
      this.showMessage(
        "Error fetching lyrics. Please enter them manually.",
        "error"
      );
    } finally {
  ...
  async fetchLyricsFromAPI(artist, title) {
    try {
      // Using lyrics.ovh API - free and no auth required
      ...
      // Fallback to alternative API or method
      try {
        return await this.fetchLyricsAlternative(artist, title);
      } catch (fallbackError) {
  ...
  async fetchLyricsAlternative(artist, title) {
    // Alternative API or method (example: search suggestion)
    // For now, return null and suggest manual entry
    return null;
  }
